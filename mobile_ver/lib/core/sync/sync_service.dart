...
class SyncService {
  ...
  Future<void> pushPending(int userId) async {
    ...
    final operations = rows.map(_outboxFromRow).toList();
  ...
  Future<void> _applyServerRecord(
    ...
    final row = _localRowFromServer(entityType, userId, record);
    if (row == null) {
      return;
    }
  ...
  SyncOutboxOperation _outboxFromRow(Map<String, Object?> row) {
    return SyncOutboxOperation(
      ...
      payload: _decodeMap(row['payload_json']?.toString() ?? '{}'),
  ...
  Map<String, dynamic>? _localRowFromServer(
    String entityType,
    int userId,
