{{-- resources/views/endmin/broadcasts/show.blade.php --}}
@extends('layouts.app')

@section('page_title', 'Detail Broadcast')
@section('page_description', 'Pantau konten, target, dan hasil push untuk proses moderasi.')
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if (! $broadcast->isArchived())
                    <form method="POST" action="{{ route('endmin.broadcast-verifications.archive', $broadcast) }}">
                        @csrf
                        @method('PATCH')
                        <button type="submit"
                ...
                @else
                    <form method="POST" action="{{ route('endmin.broadcast-verifications.unarchive', $broadcast) }}">
                        @csrf
                        @method('PATCH')
                        <button type="submit"
                                class="rounded-lg bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-100 dark:hover:bg-emerald-500/20">
                            Unarchive
                        </button>
                    </form>
                @endif
                <form method="POST" action="{{ route('endmin.broadcast-verifications.destroy', $broadcast) }}" onsubmit="return confirm('Hapus broadcast ini secara permanen?')">
                    ...
                    @method('DELETE')
                    <button type="submit"
                            class="rounded-lg bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100 dark:bg-rose-500/10 dark:text-rose-100 dark:hover:bg-rose-500/20">
                        Hapus
                    </button>
                </form>
            </div>
        ...
        @if ($broadcast->image_path)
            <div class="mt-4">
                ...
                <img src="{{ $broadcast->image_url }}"
                     ...
                     class="max-h-96 w-full rounded-xl border border-slate-200 object-contain dark:border-slate-700">
            </div>
        @endif

        <div class="mt-4">
            <p class="mb-2 text-sm font-semibold text-slate-700 dark:text-slate-200">Isi Pesan</p>
            <div class="whitespace-pre-wrap rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-200">{{ $broadcast->body }}</div>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-800">
                    @forelse ($pushLogs as $log)
                        <tr>
                            ...
                            <td class="py-3 pr-4 text-slate-700 dark:text-slate-200">{{ $log->user?->name ?: ($log->user?->email ?: '-') }}</td>
                            <td class="py-3 pr-4">
                                @if ($log->status === 'sent')
                            ...
                            <td class="break-all py-3 pr-4 text-xs text-slate-500 dark:text-slate-400">{{ $log->endpoint ?: '-' }}</td>
