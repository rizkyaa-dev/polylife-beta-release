    ...
    <div class="space-y-6">
        <div class="bg-white border rounded-3xl shadow-sm p-6 sm:p-8 dark:bg-slate-900 dark:border-slate-800">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <p class="text-sm uppercase tracking-wide text-indigo-500 font-semibold dark:text-indigo-400">Data Akademik</p>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-slate-100">Mata Kuliah Tersimpan</h2>
                    <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">Gunakan daftar ini untuk mengisi jadwal dan kegiatan lebih cepat.</p>
                </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            @forelse($matkuls as $matkul)
                <div class="bg-white border rounded-3xl shadow-sm p-5 space-y-4 dark:bg-slate-900 dark:border-slate-800">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-slate-400">{{ $matkul->kode }}</p>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-slate-100">{{ $matkul->nama }}</h3>
                    ...
                    <dl class="text-xs text-gray-600 dark:text-slate-300 space-y-1">
                    ...
                    <div class="flex items-center gap-2 text-xs">
                        <a href="{{ route('matkul.edit', $matkul) }}"
                           class="inline-flex items-center rounded-xl border border-gray-200 dark:border-slate-700 px-3 py-1.5 font-semibold text-gray-700 dark:text-slate-200 hover:border-indigo-200 hover:text-indigo-600 dark:hover:border-indigo-500/60 dark:hover:text-indigo-400">
                            Edit
                        </a>
            ...
            @empty
                <div class="bg-white border rounded-3xl shadow-sm p-8 text-center space-y-2 sm:col-span-2 xl:col-span-3 dark:bg-slate-900 dark:border-slate-800">
                    <p class="text-lg font-semibold text-gray-900 dark:text-slate-100">Belum ada matkul</p>
                    <p class="text-sm text-gray-500 dark:text-slate-400">Tambahkan matkul untuk mempercepat pembuatan jadwal dan kegiatan.</p>
                </div>
            @endforelse
        </div>
    </div>
